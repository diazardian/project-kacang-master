...
class Users(commands.Cog):
	...
	async def profile(self, ctx, *, user: discord.Member = None):
		if user is None:
			...
			embed = discord.Embed(title = f"Data Kependudukan Warga {ctx.guild.name}",color = 0x00FF00, description = "Data diri dari : " + user.mention)
			# embed.set_author(name=str(user), icon_url=user.avatar_url)
			embed.set_thumbnail(url=user.avatar_url)
			embed.add_field(name="Bergabung di server ini", value=user.joined_at.strftime(date_format))
			members = sorted(ctx.guild.members, key=lambda m: m.joined_at)
			# embed.add_field(name="Join position", value=str(members.index(user)+1))
			embed.add_field(name="Terdaftar di discord", value=user.created_at.strftime(date_format))
		if len(user.roles) > 1:
			role_string = ' '.join([r.mention for r in user.roles][1:])
			embed.add_field(name="Kekuasaan saat ini [{}]".format(len(user.roles)-1), value=role_string, inline=False)
	...
	async def con(self, ctx, *, user: discord.Member = None):
		...
		embed = discord.Embed(title = f"{user.name}'s, Heatlh Condition", description = f"Kondisi kesehatan harian kamu adalah **{health}%**", color = discord.Color.green())
		# embed.add_field(name = "Health", value = "Kosong")
		embed.set_footer(text = f"Project Kacang 1.0.0 | Under Development by Kacang Team")
